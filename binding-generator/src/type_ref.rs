...
//	}
		...
		match self.kind() {
			...
			Kind::Pointer(inner) | Kind::Reference(inner) => {
				if inner.is_void() {
					format!(
						"*{cnst}c_void",
						cnst=self.rust_const_qual(true), // fixme, change it to raw pointer or handle lifetimes
					).into()
				} else {
					format!(
						"&{lt}{cnst}{typ}",
	...
	pub fn rust_arg_func_decl(&self, name: &str) -> String {
		let typ = 'decl_type: loop { // fixme when named blocks are introduced in Rust
			if self.is_string() {
				if self.is_output() {
