// todo check if build with system headers is successful
...
mod generator {
	...
	pub fn gen_wrapper(opencv_header_dir: &Path) -> Result<()> {
		...
		let include_exts = &[OsStr::new("rs"), OsStr::new("cpp"), OsStr::new("hpp"), OsStr::new("toml"), OsStr::new("lock")];
...

		{
			...
			for module in modules {
				if !CORE_MODULES.contains(module.as_str()) {
...
fn install_wrapper() -> Result<()> {
	...
	for entry in glob(&format!("{}/*.h", cpp_hub_dir.to_str().unwrap()))? {
		let entry = entry?;
		file_copy_to_dir(&entry, &OUT_DIR)?;
	}
